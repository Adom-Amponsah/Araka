...
export function SelfieScreen() {
  ...
  const completeSelfie = useAppStore((state) => state.completeSelfie);
  const backToIdUpload = useAppStore((state) => state.backToIdUpload);
  const updateSignupData = useAppStore((state) => state.updateSignupData);
  const savedUri = useAppStore((state) => state.signupData?.selfieUri);
  const savedSource = useAppStore(
    (state) => state.signupData?.selfieSource,
  );
    ...
    try {
      ...
      if (!result.didCancel && result.assets && result.assets.length > 0) {
        ...
        if (uri) {
          setImageUri(uri);
          setSource('library');
          updateSignupData({selfieUri: uri, selfieSource: 'library'});
        }
      }
    } catch (err) {
    ...
    try {
      ...
      if (!result.didCancel && result.assets && result.assets.length > 0) {
        ...
        if (uri) {
          setImageUri(uri);
          setSource('camera');
          updateSignupData({selfieUri: uri, selfieSource: 'camera'});
        }
      }
    } catch (err) {
  ...
  return (
    <View style={styles.root}>
      <ScrollView
        ...
        bounces={false}>
        <Pressable onPress={backToIdUpload} style={styles.backButton}>
          <Ionicons name="arrow-back" size={24} color="#111827" />
        </Pressable>

        <Text style={styles.title}>Take a selfie</Text>
        <Text style={styles.subtitle}>
          For your profile to be verified, please take a correct selfie.
        </Text>
        ...
        <View style={styles.previewBox}>
          {imageUri ? (
            ...
            <>
              ...
              <Text style={styles.previewText}>Selfie preview</Text>
            </>
          )}
        </View>
